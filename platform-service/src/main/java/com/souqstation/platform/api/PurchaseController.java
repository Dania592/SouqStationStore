...
public class PurchaseController {
    ...
    /**
     * Acheter un DLC
     * POST /platform/purchases/dlc?userId=U1&dlcId=D1
     */
    @PostMapping("/dlc")
    ...
    /**
     * Récupérer la bibliothèque de DLCs d'un utilisateur
     * GET /platform/purchases/dlc-library?userId=U1
     */
    @GetMapping("/dlc-library")
    public ResponseEntity<Map<String, Object>> getUserDlcLibrary(
            @RequestParam String userId) {
        try {
            List<Map<String, Object>> library = purchaseService.getUserDlcLibrary(userId);

            return ResponseEntity.ok(Map.of(
                    "userId", userId,
                    "dlcCount", library.size(),
                    "dlcs", library));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(Map.of(
                    "status", "ERROR",
